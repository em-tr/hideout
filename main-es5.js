...
var Validation = /** @class */ (function () {
    ...
    Validation.isValidUser = function (options) {
        return function (control) {
            var selectedUser = options.filter(function (email) { return email == control.value; });
    ...
    Validation.compareValidator = function (controlNameToCompare) {
        return function (control) {
            if (control.value === null || control.value.lenght === 0) {
                return null;
            }
            var controlToCompare = control.root.get(controlNameToCompare);
            if (controlToCompare) {
                var sub_1 = controlToCompare.valueChanges.subscribe(function () {
...
var AuthService = /** @class */ (function () {
    function AuthService(chatDBService) {
        var _this = this;
        ...
        chatDBService.registerOnMessageSubject(function (event) {
            switch (event.type) {
                case "LoggedIn":
                ...
                case "LoggedOut":
                    ...
                    break;
                case "UserRenamed":
                    var renamedUser = event.value;
                    _this.currentUserSubject.next(renamedUser);
                    break;
                default:
                    break;
            }
    ...
    AuthService.prototype.registerOnUserSubject = function (callback) {
    ...
    AuthService.prototype.clearTimer = function () {
...
var ChangePWComponent = /** @class */ (function () {
    function ChangePWComponent(dialogRef, data, formBuilder, chatDBService) {
        var _this = this;
        ...
        chatDBService.registerOnMessageSubject(function (event) {
            switch (event.type) {
                case "ChangePasswordFailed":
                    _this.errorMsg = "Wrong Password, pls retry";
                    break;
                case "ChangedUserPassword":
                    _this.dialogRef.close({ passwordChanged: true });
                    break;
                default:
                    break;
            }
    ...
    Object.defineProperty(ChangePWComponent.prototype, "f", {
        get: function () { return this.changePwForm.controls; },
        enumerable: true,
        configurable: true
    });
    ChangePWComponent.prototype.onNoClick = function () {
        this.dialogRef.close({ passwordChanged: false });
    };
    ChangePWComponent.prototype.resetErrorMsg = function () {
        this.errorMsg = "";
    };
    ChangePWComponent.prototype.onSubmit = function () {
...
var ChangeUserNameComponent = /** @class */ (function () {
    function ChangeUserNameComponent(dialogRef, data, chatDBService) {
        var _this = this;
        this.dialogRef = dialogRef;
        this.data = data;
        this.chatDBService = chatDBService;
        this.chatDBService.registerOnMessageSubject(function (event) {
            switch (event.type) {
                case "UserRenamed":
                    _this.dialogRef.close({ email: event.value.email, name: event.value.name });
                    break;
                default:
                    break;
            }
    ...
    ChangeUserNameComponent.prototype.getErrorMessageName = function () {
        return this.nameForm.hasError('required') ? 'You must enter a value' :
            this.nameForm.hasError('pattern') ? 'pattern asdf_dd' :
                '';
    };
    ChangeUserNameComponent.prototype.onNoClick = function () {
        this.dialogRef.close();
    };
    ChangeUserNameComponent.prototype.onSubmit = function () {
        var newName = this.nameForm.value;
...
var ChatBoxComponent = /** @class */ (function () {
    function ChatBoxComponent(chatDBService, authService) {
        this.chatDBService = chatDBService;
        this.authService = authService;
        this.messages = [];
        this.roomMessages = [];
    ...
    ChatBoxComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.room.subscribe(function (room) { _this.update(); });
        this.chatDBService.registerOnMessageSubject(function (event) {
            switch (event.type) {
                case "MessageSendToRoom":
                    _this.messages.push({ email: event.value.email, message: event.value.message, roomName: event.value.roomName, userName: event.value.userName, time: new Date() });
                    _this.update();
                    break;
                case "UserRenamed":
    ...
    ChatBoxComponent.prototype.sendMessage = function (text) {
        var currentUserRights = this.getCurrentUserByRoom(this.room.value);
        var room = this.getRoomByRoomName(this.room.value);
    ...
    ChatBoxComponent.prototype.updateRoomMessages = function () {
        var _this = this;
        this.roomMessages = this.messages.filter((function (message) {
            return message.roomName == _this.room.value;
        }));
    };
    ChatBoxComponent.prototype.updateFinalMessages = function () {
        var _this = this;
        var index = 0;
        var lastMessage = null;
        this.finalMessages = [];
        this.roomMessages.forEach(function (roomMessage) {
            if (_this.finalMessages[index] == null) {
                _this.finalMessages[index] = [];
            }
            if (lastMessage != null && (roomMessage.email != lastMessage.email || (roomMessage.time.getTime() - _this.finalMessages[index][0].time.getTime()) >= (1000 * 60 * 60 * 24) || (roomMessage.time.getDate() - _this.finalMessages[index][0].time.getDate()) >= 1)) {
                ++index;
                _this.finalMessages[index] = [];
            }
            _this.finalMessages[index].push(roomMessage);
            lastMessage = roomMessage;
        });
    };
    ChatBoxComponent.prototype.update = function () {
        this.updateRoomMessages();
        this.updateFinalMessages();
    };
    ChatBoxComponent.prototype.getCurrentUserByRoom = function (room) {
        var _this = this;
        if (this.usersWithRoomAndRights.value == null) {
            return null;
        }
        return this.usersWithRoomAndRights.value.find(function (user) {
            return user.roomName == room && user.email == _this.authService.currentUserValue.email;
        });
    };
    ChatBoxComponent.prototype.getRoomByRoomName = function (roomName) {
...
var ChatDBService = /** @class */ (function () {
    function ChatDBService() {
        var _this = this;
        ...
        this.connection.onmessage = function (e) {
            ...
            _this.subject.next(o);
        };
        this.registerOnMessageSubject(function (event) {
            switch (event.type) {
                case "LoggedIn":
                    _this.pushSubjectValue(event.value.email);
                    _this.InformLogin();
                    break;
                case "LoginInformEvent":
                    if (!_this.onlineUsersSubject.value.find(function (user) {
                ...
                case "LogoutInformEvent":
                    _this.onlineUsersSubject.next(_this.onlineUsersSubject.value.filter(function (user) {
                        return user != event.value.email;
                    }));
                    break;
                default:
    ...
    ChatDBService.prototype.registerOnMessageSubject = function (callback) {
        this.subject.subscribe(callback);
    };
    //Sendet die Events an das Backend
    ChatDBService.prototype.sendEvent = function (type, data) {
        var _this = this;
        var command = {
            type: type,
            value: data
    ...
    ChatDBService.prototype.stableLogin = function (user, timeInMs) {
        var _this = this;
        if (timeInMs === void 0) { timeInMs = 10000; }
        this.tryLogin(user);
    ...
    ChatDBService.prototype.tryLogin = function (user) {
        this.sendEvent("Login", user);
    };
    //Betritt einen Raum
    ChatDBService.prototype.joinRoom = function (room) {
        this.sendEvent("JoinRoom", { "roomName": room });
    };
    //Verlässt einen Raum
    ChatDBService.prototype.leaveRoom = function (room) {
        this.sendEvent("LeaveRoom", { "roomName": room });
    };
    //Definiert für einen Raum, ob eine Einladung erforderlich ist
    ChatDBService.prototype.setInviteRoom = function (roomName, inviteRequired) {
...
var ChatMainComponent = /** @class */ (function () {
    ...
    ChatMainComponent.prototype.ngOnInit = function () {
    };
...
var ChatRoomSelectComponent = /** @class */ (function () {
    function ChatRoomSelectComponent(chatDBService, authService) {
        this.chatDBService = chatDBService;
        this.authService = authService;
    ...
    ChatRoomSelectComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.roomsEventEmitter.next(this.rooms);
        this.usersWithRoomAndRights.subscribe(function (users) {
            if (users == null) {
                return;
            }
        ...
        this.chatDBService.registerOnMessageSubject((function (event) {
            switch (event.type) {
                ...
                case "InvitedOfRoomRequired":
                    _this.changeInviteOfRoomRequired(event.value.roomName, event.value.inviteRequired);
                    break;
                case "VoiceInRoomRequired":
                    _this.changeVoiceInRoomRequired(event.value.roomName, event.value.voice);
                    break;
            }
        }));
    };
    ChatRoomSelectComponent.prototype.changeInviteOfRoomRequired = function (roomName, inviteRequired) {
        this.getRoomByRoomName(roomName).inviteRequired = inviteRequired;
    };
    ChatRoomSelectComponent.prototype.changeVoiceInRoomRequired = function (roomName, voice) {
        this.getRoomByRoomName(roomName).voice = voice;
    };
    ChatRoomSelectComponent.prototype.newRoomValue = function (room) {
        this.room.next(room);
        this.currentRoom = room;
        this.chatDBService.joinRoom(room);
    };
    ChatRoomSelectComponent.prototype.leaveRoomButtonClicked = function (room) {
        this.chatDBService.leaveRoom(room);
    };
    ChatRoomSelectComponent.prototype.getCurrentUserByRoom = function (room) {
        var _this = this;
        if (this.usersWithRoomAndRights.value == null) {
            return null;
        }
    ...
    ChatRoomSelectComponent.prototype.getRoomByRoomName = function (roomName) {
        return this.rooms.find(function (room) {
            return room.roomName == roomName;
        });
    };
    ...
    return ChatRoomSelectComponent;
}());
...
var InviteComponent = /** @class */ (function () {
    function InviteComponent(dialogRef, data, dbService) {
        var _this = this;
        this.dialogRef = dialogRef;
        this.data = data;
        ...
        this.options = [];
        this.dbService.registerToOnlineUsersSubject(function (usersMailsOnline) {
            _this.getUserMails();
            _this.options = usersMailsOnline.filter(function (val) { return !_this.userMailsInRoom.includes(val); });
        });
    }
    ...
    InviteComponent.prototype._filter = function (value) {
        var filterValue = value.toLowerCase();
        return this.options.filter(function (option) { return option.toLowerCase().includes(filterValue); });
    };
    InviteComponent.prototype.getUserMails = function () {
        var _this = this;
        this.data.users.forEach(function (val) {
            _this.userMailsInRoom.push(val.email);
        });
    };
    InviteComponent.prototype.getErrorMessage = function () {
        return this.myControl.hasError('selectedUser') ? 'Select user' :
            '';
    };
    InviteComponent.prototype.onNoClick = function () {
        this.dialogRef.close();
    };
    InviteComponent.prototype.onSubmit = function () {
        var userToInvite = this.myControl.value;
...
var LoginComponent = /** @class */ (function () {
    function LoginComponent(formBuilder, route, router, authService, chatDBService) {
        var _this = this;
        this.formBuilder = formBuilder;
        this.route = route;
        this.router = router;
        ...
        this.chatDBService.registerOnMessageSubject(function (event) {
            if (event.type == "LogginFailed") {
                _this.errorMsg = "Login Failed, pls retry";
                _this.authService.clearTimer();
            }
        });
    }
    LoginComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.unsub = this.authService.currentUser.subscribe(function (user) {
            if (user) {
    ...
    LoginComponent.prototype.onSubmit = function () {
        var userCredentials = { password: this.f.password.value, email: this.f.email.value };
        this.authService.login(userCredentials);
    };
    LoginComponent.prototype.ngOnDestroy = function () {
        this.unsub.unsubscribe();
    };
    LoginComponent.prototype.resetErrorMsg = function () {
        this.errorMsg = "";
    };
    LoginComponent.prototype.getErrorMessageEmail = function () {
...
var PeopleInRoomComponent = /** @class */ (function () {
    function PeopleInRoomComponent(chatDBService, authService) {
        this.chatDBService = chatDBService;
        this.authService = authService;
        this.usersWithRoomAndRights = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
        this.users = [];
    }
    PeopleInRoomComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.chatDBService.registerOnMessageSubject((function (event) {
            var user;
            switch (event.type) {
                case "RoomJoined":
                    _this.users.push({ name: event.value.name, email: event.value.email, roomName: event.value.roomName, op: false, voice: true });
                    _this.update();
                    break;
                case "RoomLeft":
                    if (_this.authService.currentUserValue.email == event.value.email) {
                ...
                case "OpGranted":
                    user = null;
                    user = _this.users.find(function (user) {
                ...
                case "UserRenamed":
                    ...
                    break;
                case "KickEvent":
                    _this.leaveRoomBecauseOfKick(event.value.roomName, event.value.email);
                    break;
                case "InvitedToRoom":
    ...
    PeopleInRoomComponent.prototype.leaveRoomBecauseOfKick = function (roomName, email) {
        if (this.authService.currentUserValue.email == email)
            this.chatDBService.leaveRoom(roomName);
    };
    PeopleInRoomComponent.prototype.joinRoomBecauseOfInvite = function (roomName, userEmail) {
        if (userEmail == this.authService.currentUserValue.email) {
            this.chatDBService.joinRoom(roomName);
        }
    };
    PeopleInRoomComponent.prototype.getCurrentUserForRoom = function (room) {
        var _this = this;
        return this.users.find(function (user) {
            return user.email == _this.authService.currentUserValue.email && user.roomName == room;
        });
    };
    PeopleInRoomComponent.prototype.update = function () {
...
var RegisterComponent = /** @class */ (function () {
    function RegisterComponent(formBuilder, router, chatDBService) {
        var _this = this;
        this.formBuilder = formBuilder;
        this.router = router;
        this.chatDBService = chatDBService;
    ...
    RegisterComponent.prototype.onSubmit = function () {
        var userCredentials = { password: this.f.password1.value, email: this.f.email.value, name: this.f.name.value };
    ...
    RegisterComponent.prototype.getErrorMessageName = function () {
    ...
    RegisterComponent.prototype.getErrorMessagePW = function () {
...
var RoomSettingsComponent = /** @class */ (function () {
    function RoomSettingsComponent(chatdb, dialog) {
        this.chatdb = chatdb;
        this.dialog = dialog;
    }
    RoomSettingsComponent.prototype.ngOnInit = function () {
        this.currentRoomAndUsers = this.room;
        console.log(this.room);
    };
    RoomSettingsComponent.prototype.setRommPrivate = function () {
        this.chatdb.setInviteRoom(this.room.roomName, true);
        this.inviteToRoom(this.currentUser.email);
    };
    RoomSettingsComponent.prototype.setRoomPublic = function () {
        this.chatdb.setInviteRoom(this.room.roomName, false);
    };
    RoomSettingsComponent.prototype.revokeVoiceFromRoom = function () {
        this.chatdb.setVoiceRoom(this.room.roomName, false);
    };
    RoomSettingsComponent.prototype.setVoiceToRoom = function () {
        this.chatdb.setVoiceRoom(this.room.roomName, true);
    };
    RoomSettingsComponent.prototype.inviteToRoom = function (mail) {
        this.chatdb.inviteToRoom(this.room.roomName, mail, true);
    };
    RoomSettingsComponent.prototype.leaveRoom = function () {
        this.chatdb.leaveRoom(this.room.roomName);
    };
    RoomSettingsComponent.prototype.openInviteDialog = function () {
        var _this = this;
        var dialogRef = this.dialog.open(_invite_invite_component__WEBPACK_IMPORTED_MODULE_4__["InviteComponent"], {
            width: '250px',
            data: this.currentRoomAndUsers
        });
        dialogRef.afterClosed().subscribe(function (result) {
            if (result) {
                _this.inviteToRoom(result);
            }
        });
    };
    tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
...
var SiedebarComponent = /** @class */ (function () {
    function SiedebarComponent() {
        this.title = 'chatroom';
        this.admins = [
            { name: 'user1', state: 'online', status: 'carpe diem' },
            { name: 'user2', state: 'offline', status: 'hello world' },
    ...
    SiedebarComponent.prototype.onSidenavToggle = function () {
    ...
    SiedebarComponent.prototype.ngOnInit = function () {
    };
...
var UserTypeComponent = /** @class */ (function () {
    ...
    UserTypeComponent.prototype.getColor = function (user) {
    ...
    UserTypeComponent.prototype.getUserType = function (user) {
        if (!user.voice && user.op) {
            return "OP, No Voice";
        }
        if (user.op && user.voice) {
...
var UsersettingsComponent = /** @class */ (function () {
    function UsersettingsComponent(authService, dialog, router) {
        this.authService = authService;
        this.dialog = dialog;
        this.router = router;
    }
    ...
    UsersettingsComponent.prototype.ngOnDestroy = function () {
        this.unsub.unsubscribe();
    };
    ...
    UsersettingsComponent.prototype.openDialogChangePW = function () {
        var _this = this;
        var dialogRef = this.dialog.open(_change_pw_change_pw_component__WEBPACK_IMPORTED_MODULE_4__["ChangePWComponent"], {
            width: '250px',
    ...
    UsersettingsComponent.prototype.openDialogChangeUserName = function () {
        var dialogRef = this.dialog.open(_change_user_name_change_user_name_component__WEBPACK_IMPORTED_MODULE_5__["ChangeUserNameComponent"], {
            width: '250px',
            data: this.currentUser
        });
        dialogRef.afterClosed().subscribe(function (result) {
